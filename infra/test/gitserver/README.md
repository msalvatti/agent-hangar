...
the bare repository `/repos/e2e/sample.git` (branches `main` and `feature/docs`) with fixed author
and commit dates, so the seed commits have the same SHAs on every machine. It lives under an owner
directory because every repository URL in this product has the owner-and-repository shape.
...
Repository URLs have the shape `http://<host>:<port>/<owner>/<name>.git` — for example
`http://127.0.0.1:3907/e2e/sample.git` from the host, or
`http://host.docker.internal:3907/e2e/sample.git` from inside a workspace container.
