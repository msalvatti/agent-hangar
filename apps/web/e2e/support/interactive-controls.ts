...
/**
 * Every interactive control on the page whose own box is taken, somewhere inside it, by a
 * different element.
 *
 * @param page - The page to measure, already at rest.
 * @returns One entry per covered control, empty when every control takes its own clicks.
 */
export async function findCoveredControls(page: Page): Promise<Collision[]> {
  await settleAnimations(page);
  return page.evaluate((selector: string) => {
    /** The screen-reader-only clip rectangle, as `getComputedStyle` reports it. */
    const CLIPPED_AWAY = 'rect(0px, 0px, 0px, 0px)';
    /**
     * Where inside a control's box the sample points are taken, as fractions of its width and its
     * height, crossed with themselves into a three-by-three grid.
     *
     * The centre alone is not enough, and the collision that motivated all of this proves it: at
     * 1440 px the run drawer's Copy button and the sheet's close button overlapped in a nine-pixel
     * band that contained neither centre, so a centre-only rule reported nothing while a click
     * near the top of Copy still closed the drawer. Every one of these points lies strictly inside
     * the control, so each of them belongs to the control or to a descendant unless something else
     * is drawn over it — which is the whole of the rule.
     */
    ...
    /**
     * Whether a point is inside every clipping ancestor of an element, and therefore a point at
     * which the element is actually painted.
     */
...
/**
 * Renders collisions as the lines a failing assertion prints.
 *
 * Both elements are named, because which one is on top is the whole of the diagnosis: the fix
 * moves one of them, and the message has to say which two are fighting over the point.
 *
 * @param where - What was on screen, so a failure names the screen as well as the controls.
 * @param collisions - What {@link findCoveredControls} measured.
 * @returns One line per collision.
 */
export function describeCollisions(where: string, collisions: readonly Collision[]): string[] {
...
/**
 * Every control on the page that a click activates but the cursor does not offer to.
 *
 * The design rules require a pointer cursor on every interactive element, and a class assertion
 * cannot establish it: the failure this catches is not a missing class but a losing one. The
 * shadcn registry ships menu rows and palette options with `cursor-default`, so the class was
 * present, the utility resolved, the element rendered — and the row looked inert under the
 * pointer. Only the computed style says which one won.
 *
 * The same exclusions as {@link findCoveredControls} apply: a control hidden from assistive
 * technology, clipped to the screen-reader box or given no pointer events at all is not a target
 * this rule has anything to say about.
 *
 * @param page - The page to measure.
 * @returns One description per control, empty when every one of them offers a pointer.
 */
export async function findControlsWithoutPointerCursor(page: Page): Promise<string[]> {
  return page.evaluate((selector: string) => {
    /** The screen-reader-only clip rectangle, as `getComputedStyle` reports it. */
    const CLIPPED_AWAY = 'rect(0px, 0px, 0px, 0px)';
    /** How much of an element's own text is kept when naming it in a failure message. */
    ...
    for (const element of document.querySelectorAll(selector)) {
      ...
      const rect = element.getBoundingClientRect();
      if (rect.width === 0 || rect.height === 0 || style.cursor === 'pointer') {
        continue;
      }
      const name =
        element.getAttribute('aria-label') ?? element.textContent.replace(/\s+/g, ' ').trim();
      ...
      found.push(
        `${element.tagName.toLowerCase()}${role === null ? '' : `[role="${role}"]`}` +
          `${name === '' ? '' : ` "${name.slice(0, NAME_BUDGET)}"`} has cursor: ${style.cursor}`,
      );
    }
    return found;
  }, CLICKABLE_SELECTOR);
