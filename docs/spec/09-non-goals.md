...
|---|---|---|
| **Multi-user authentication** | The app runs locally for one developer; auth adds a login flow, session storage, and per-user data partitioning with no local benefit. | Route handlers receive a `RequestContext`; repositories take it as a parameter; `Secret` keyed by `key` today becomes `(userId, key)` with one migration. OIDC via `next-auth`/`proxy.ts` drops in at the edge. |
