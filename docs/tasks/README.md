...
**Where the build stands (2026-08-23).** Fifteen of the seventeen lanes are merged. W3-A, wiring and stabilisation, closed with all six of its tasks; the two mutation-testing lanes are 🟩 done and held on a local branch at the operator's instruction — the work exists and passes, and nothing about it is on the remote. Their scope grew past the two packages they name: every package plus `infra/scripts/lib` scores 100.00, breaking below it (see [plan §9](../plan.md)). One success criterion — the ten-minute first run on a *clean* machine — is recorded 🟡 partial: it was measured at 21 seconds, but on a warm one, and a cold dependency install and image build are the costs that measurement cannot cover.
