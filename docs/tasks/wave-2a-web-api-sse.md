...
- Superseded ✅ 2026-08-21 — the replay contract above is the record of what this lane built, and R49 has since narrowed it. The pump no longer replays unconditionally from `Last-Event-ID`: it first asks the stream whether it still holds that entry (`xrange(key, id, id)`), and answers `event: expired` and closes when it does not, because the cap can trim past a client's position while the key is very much alive. So `xrange` is called twice on a resuming connection, with different arguments, and the recorder assertion described in the test list characterises only the second. `runDetail` also carries `push: { branch, sha } | null` now (R46).
