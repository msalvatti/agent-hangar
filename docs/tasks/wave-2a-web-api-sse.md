...
1. `apps/web/src/server/sse.ts`:
   ...
   d. Loop while `!closed`: `const res = await conn.xread('BLOCK', opts.blockMs, 'STREAMS', streamKey, cursor)`; `null` → if `await opts.isFinished()` and `cursor !== '0-0'` → close (the turn ended and everything was delivered — belt and braces in case the terminal event was never written), else continue; otherwise for each `[id, fields]`: `const ev = parseTurnEventEntry(fields)`; if `ev === null` → `write(formatSseFrame({ id, event: 'protocol.error', data: JSON.stringify({ type: 'protocol.error', line: '', reason: 'unparseable stream entry' }) }))`; else `write(formatSseFrame({ id, event: ev.type, data: JSON.stringify(ev) }))`; `cursor = id`; if `SSE_TERMINAL_EVENTS.includes(ev.type)` → close and return.
