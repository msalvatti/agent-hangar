...
- [x] `restorationNotice({ at, workBranch })` with workBranch → exactly `` Workspace recreated from history at <time>. Uncommitted changes from the previous workspace are gone; pushed work on `<workBranch>` is checked out. ``; without → `Workspace recreated from history at <time>. Uncommitted changes from the previous workspace are gone; no pushed work was found, so the base branch is checked out.`
