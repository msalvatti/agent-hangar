...
- Superseded ✅ 2026-08-21 — two of this lane's behaviours were replaced after R10 and R39 closed, so the criteria and prompts above are the record of what was built here, not of what the tree now does. The boot probe is no longer `runner.list` proving reachability with the image check deferred (2B.4, and its prompt): `probeWorkspaceImage` asks `runner.imageExists(config.WORKSPACE_IMAGE)`, takes no instance, and the contract change request this lane was told to file was granted long ago. The heartbeat no longer publishes a remembered image status — `apps/worker/src/image-status.ts` is deleted — and asks the runner on every beat. The collector has a third reconciliation arm, `finishAbandonedTeardowns`, so `GcResult` is `{ reaped, orphansDestroyed, goneMarked, teardownsFinished }` rather than the three-field shape named above, and a `STOPPING` row whose container is still running is now reclaimed in the steady state. `git.pushed` inside a scheduled run is persisted (R46) instead of discarded: `jobRuns.recordPush`.
