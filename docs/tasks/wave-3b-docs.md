...
- Review response ✅ 2026-08-20 — eight PR #37 review threads verified and closed: the scripts legend now names the two script styles and marks `start`/`test:integration` 🐳 while `build`/`test`/`test:mutation` are marked Docker-free; the doctor db-name example corrected to `agent_hangar_feat_x` (verified by running `infra/scripts/doctor.sh` with `AH_INSTANCE=feat-x`); the authentication seam in README and `docs/spec/09-non-goals.md` corrected — `ServerContainer` is a process-wide singleton (`getServerContainer()`), not a per-caller seam, so the `Request` every route handler already receives is named instead; `docs/spec/README.md`'s revision line corrected to say the 06 success criteria were raised to match the enforced configuration rather than left unchanged; `docs/spec/05-local-dev.md`, `06-testing.md` and `09-non-goals.md` each gained their own `Status`/`Revision` line; README gained the screenshot and commented-out mutation-badge placeholders, the `prepare` script row, and the Testing table's `Budget` column
