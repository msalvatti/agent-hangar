...
**Status:** 📋 ToDo · **Priority:** P0 · **Size:** L · **Depends on:** 1H.3
...
**Acceptance criteria**
- [ ] `/scheduled/<id>` header: name (h1), `ScheduleCell`, enabled `Switch`, "Run now" button (busy spinner; 409 → toast "Skipped: previous run still running"), overflow Edit (opens `JobDialog`) / Delete (dialog → navigate to `/scheduled`); 404 → `ErrorCard` "Job not found" + link
...
- [ ] `RunDrawer` (Sheet right, `w-[720px] max-w-[100vw]`): header with job name, started time, `StatusPill`, Stop button while active (confirm `AlertDialog` → `POST /api/turns/:id/cancel` with the run id — TurnRequest.turnId is the JobRun id; note if contracts.ts has a dedicated run cancel route), Copy run id; `Tabs`: Transcript (shared `Transcript readOnly` fed by `mapRunDetail` items + `useTurnEvents` while active; reconnect bar text when `reconnecting`) and Raw output (`pre` mono with `run.output` or "No output yet." + copy); Esc closes; focus trapped and returned
- [ ] 100 % coverage on `src/features/scheduled/**`; `JobDetailView` exported from the barrel; page thin
...
**Acceptance criteria**
- [ ] `/settings` shows both cards; `SecretField` states: loading skeleton · unset (password input, Save disabled until non-empty) · set (mask `••••••••<last4>` mono + "updated 3 days ago" + Replace/Remove) · replacing (input + Save/Cancel) · saving (spinner, disabled) · error (inline under the field, `aria-describedby`); after save the input is cleared and the mask shows the new last4; canary plaintext never appears in the DOM after save (test)
- [ ] Remove → `AlertDialog` "Remove <label>?" body "Workspaces will start without it until you add a new one." Cancel/Remove → `DELETE` → toast "<label> removed" → unset state
- [ ] Helper texts: GitHub — "Needs repo scope (read + push) for the repositories you want to use."; both — "Stored encrypted on this machine. Only sent to GitHub / OpenAI." (card description: "Stored encrypted on this machine. Injected into workspaces at start."); inputs `autoComplete="off"`, `spellCheck=false`, visible labels
- [ ] Model line "Model <mono id> (from OPENAI_MODEL)"; `EnvSummary` renders instance, ports and each dependency as icon + text from `healthResponse`; infra-down scenario → destructive items; loading skeleton; error → `ErrorCard` + Retry
- [ ] 100 % coverage on `src/features/settings/**`; `coverage.include` extended; page thin; Lighthouse a11y ≥ 95 on `/settings`
