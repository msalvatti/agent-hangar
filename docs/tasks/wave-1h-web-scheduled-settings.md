...
- Superseded ✅ 2026-08-21 — the run drawer rebuilds one more row than this lane's mapper produced. R46 gave `JobRun` a push record (`workBranch`/`lastPushedSha`), `runDetail` carries it as `push`, and `mapRunDetail` turns it into the same `Pushed <branch> @ <sha>` notice the live stream shows, placed between the tool calls and the output. The drawer's expiry recovery also reconnects with `fromStart` now, for the reason recorded in W1-G's log.
