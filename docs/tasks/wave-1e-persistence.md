...
**Acceptance criteria**
- [x] `mappers.ts` exports one `to<Entity>(row)` function per model (`toChat`, `toMessage`, `toTurn`, `toWorkspace`, `toScheduledJob`, `toJobRun`, `toToolCallLog`, `toSecretRecord` — the `Secret` row maps to the `SecretRecord` of `ports.ts`, not to a `SecretEnvelope`) returning the domain types used by `ports.ts`, with `null` → `undefined` conversion where the domain type uses optional fields (`exactOptionalPropertyTypes`) and `Date` values passed through untouched
...
- [x] `errors.ts` re-exports the three error kinds `src/errors.ts` already defines for the in-memory doubles — `NotFoundError`, `LiveWorkspaceExistsError` (field `chatId`), `UniqueViolationError` (fields `entity`, `constraint`) — so a caller written against either repository catches the same type, and adds the only kind new to this layer, `PersistenceMappingError` (`code: 'PERSISTENCE_MAPPING'`), extending `AgentHangarError`. The originally specified `EntityNotFoundError` / `LiveWorkspaceConflictError` names were dropped: defining them would have split each failure into two incompatible types.
- [x] `prisma-errors.ts` exports `translatePrismaError(error: unknown, ctx): never` mapping `P2002` → `LiveWorkspaceExistsError` when the violated index is `Workspace_one_live_per_chat`, otherwise `UniqueViolationError`; `P2003` (foreign-key violation — what Postgres actually raises for a missing parent) → `NotFoundError` naming the parent; `P2025` → `NotFoundError`; anything else is rethrown unchanged
- [x] `index.ts` exports `createRepositories(prisma, redactor): Repositories` (property names identical to `createInMemoryRepositories` in `src/testing/in-memory-repositories.ts`) plus every class and error, and `persistence/index.ts` re-exports the composition root and the classes so they are reachable from `@agent-hangar/core`; the single Prisma-typed thing on that surface is the `PrismaClient` parameter of `createRepositories`, the same allowed leak as `createPrismaClient`
...
- Superseded ✅ 2026-08-21 — `JobRunRepository` has one method more than this lane built: `recordPush(id, { workBranch, lastPushedSha })`, added for R46, with both implementations held to the shared contract in `packages/core/src/persistence/testing/job-run-push-contract.ts`. `JobRun` carries the two matching nullable columns (migration `0002_job_run_push_record`, additive).
