...
| R7 | `healthResponse` does **not** carry `ports` today — the field exists only on the web API lane's unmerged branch, where it is required. The moment that branch merges, `seedHealth()` in `apps/web/src/mocks/store.ts` stops typechecking, because the field becomes required and that branch does not touch the file. The Environment card also cannot render the ports line its wireframe describes until then. | A contract change that breaks a caller is the changing lane's to repair, so it belongs in that lane's rebase rather than in the lane that would be broken. Recorded because an earlier version of this row stated the field as already required, which it is not. | W2-A |
