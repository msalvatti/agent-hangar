...
| R7 | The web API lane's branch declares `ports` on `healthResponse` as **required**. Its own lane rules permit additive changes to existing response schemas only — new optional fields — and its task sheet says in as many words to add missing health fields as optional. So the required declaration is itself the violation, and it is what would break `seedHealth()` in another lane's mocks on merge. | Making the field optional satisfies the lane's own rule, keeps every existing consumer parsing, and means that lane never has to reach into mocks it does not own. Repairing the caller instead would have preserved the breaking change and pushed the lane across an ownership boundary to do it. | W2-A |
