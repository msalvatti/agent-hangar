 ...
 * own, which is exactly what `touch` sends); `delete`/`update` failures translate through
...
describe('PrismaChatRepository', () => {
  ...
  it('rename() updates the title', async () => {
    ...
    expect(chat.update).toHaveBeenCalledWith({
      where: { id: 'chat-1' },
      data: { title: 'New title', updatedAt: expect.any(Date) as Date },
    });
  });
