...
The doctor derives the instance from your **shell**, not from `.env.local`. So do `ws:list`, `ws:reap`, `db:prune`, `archive` and `rotate-key`; only `setup` and `dev` read the file. Give them the same prefix you gave `pnpm setup` — `AH_INSTANCE=feat-x AH_PORT_BASE=3100 pnpm run doctor` — or they act on `default`.
...
| **Five scripts read the instance from the shell, not from `.env.local`** — `doctor`, `ws`, `archive`, `db:prune` and `rotate-key` — while `setup` and `dev` read the file.                  | Run in a non-default checkout without the prefix, the doctor reports on `default`, and `pnpm archive` tears down `default`'s stack while still deleting this worktree's `.env.local`. Always pass the same `AH_INSTANCE`/`AH_PORT_BASE` you set the checkout up with. |
