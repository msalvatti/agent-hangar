...
| `[WARN] Unsupported engine` during the install                    | You are not on Node 24 and the install continues regardless — the range is not enforced. Nothing here is tested on another major, so switch before going further: `nvm use` or `fnm use` reads `.nvmrc`.                                |
...
| **No check in this repository can see one interactive control covering another** (R43). Every `getBoundingClientRect` in jsdom is 0×0, so an overlap assertion written there passes whatever the layout does. A collision between two controls is visible to a person opening the page and to nothing else.                 | Layout regressions of that class reach the running product.                                                                                                       |
...
| **The Node requirement is a warning, not a gate.** `engines` asks for `>=24 <25` and `.npmrc` sets `engine-strict=true`, but pnpm 11 does not read that setting — `pnpm config get engine-strict` answers `undefined` — so a wrong major prints `[WARN] Unsupported engine` and installs anyway. | An unsupported major is untested rather than known to break, and nothing downstream will remind you which one you are on. Check `node -v` yourself, or run `nvm use` / `fnm use`. |
