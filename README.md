...
> Use `pnpm run doctor`, not `pnpm doctor` — pnpm has a built-in `doctor` command that shadows the script and reports on your pnpm installation instead. This is a naming collision in the repository, not something you can configure away; it is listed under [Known gaps](#-known-gaps).

It prints one row per check — Node, pnpm, the Docker socket it found, PostgreSQL, Redis, migrations, the workspace image, the master key and its mode, which credentials are configured, and whether your OpenAI key can reach the configured model. **Every ✗ comes with the exact command that fixes it**, and the exit code is non-zero only when a required row failed. `pnpm run doctor --json` prints the same rows as JSON.

The doctor derives the instance from your **shell**, not from `.env.local`. If you set up a non-default instance, give it the same prefix you gave `pnpm setup` — `AH_INSTANCE=feat-x AH_PORT_BASE=3100 pnpm run doctor` — or it will report on `default` and look wrong.
...
| ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `pnpm doctor` reports on pnpm, not on Agent Hangar            | pnpm's built-in `doctor` shadows the script. Run `pnpm run doctor`.                                                                                                                                                      |
...
| **The image-presence check is optimistic before the first run** (R10). `WorkspaceRunner` exposes no `imageExists`, so the boot check and the health card report what the last `create` observed.                   | The health card can show a green image before anything has ever been created. `pnpm run doctor` asks Docker directly.                                                      |
...
| Item                                                                                                                                                                                           | Residual risk                                                                                                                                   |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **`pnpm doctor` runs pnpm's own doctor.** The script needs `pnpm run doctor`; renaming it is the real fix.                                                                                     | A reader who copies `pnpm doctor` gets a report about their pnpm installation and no error.                                                     |
| **The doctor reads the instance from the shell, not from `.env.local`**, while `setup` and `dev` read the file. The three scripts that use the shell form are `doctor`, `ws` and `rotate-key`. | Run in a non-default checkout without the prefix, the doctor reports on `default`.                                                              |
| **The PostgreSQL and Redis rows are TCP-open probes**, not protocol handshakes.                                                                                                                | An unrelated listener on the port reads as ✓. The `Migrations` row, which does connect, is the one that tells you the database is really yours. |
...
Infrastructure is a rounding error next to model spend. Model cost is controlled by the step limit, output truncation, history windowing, and choosing a mid-tier model for routine scheduled jobs — which is one column on the job row.
