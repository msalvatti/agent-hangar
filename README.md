...
- **A task can read the forge token, and the model key with it.** The shell tool removes `GITHUB_TOKEN` and `OPENAI_API_KEY` from the child environment and mediates git authentication through the askpass helper, which stops the token appearing in a remote URL or in git's output. It does not put either credential out of reach, and the token file named by `AH_GIT_TOKEN_FILE` is the least of it. A task's commands run as the same user as the container's main process, so `cat /proc/1/environ` returns both values with no file involved; and the askpass helper is executable by the task — it has to be, since git only reaches it as a child of the task — so a hand-written `Password for 'https://github.com'` prompt is answered with the token. All three are measured, not inferred. A setuid helper does not close them: the container runs with `no-new-privileges`, which makes the bit inert, and a helper the task can still call is still an oracle. Nor does a credential daemon on a socket, because anything git can dial from the agent's namespace the agent can dial. The one shape that works keeps the credentials out of the container altogether and mediates git and the model call from the worker — a different process model, not a patch — and it buys little without an egress policy the workspace does not have today (`bridge` networking, `curl` in the image), which an allow-list cannot supply either, since the forge itself is a place data can be written to. So the honest statement is the one control that actually holds: **give the agent a token scoped to what you are willing to lose.**
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **A task's shell can read the credentials** (R1). Askpass mediation stops the token leaking into URLs and git output; it does not stop the agent reading `/proc/1/environ`, which returns both credentials with no file involved, reading the token file at `AH_GIT_TOKEN_FILE`, or invoking the askpass helper itself with a prompt naming the approved origin. All three are the same fact: the agent's commands run as the user the credentials were handed to.            | Scope the PAT to what you are willing to lose, and do not point the agent at prompts you have not read.                                                                                    |
| **A terminal run status still arrives before the workspace is gone** (residual of R27). Both processors persist the outcome and only then destroy the container and release the row; deleting on the strength of that status no longer fails anything, but it does not wait for the teardown either.                                                                                                                                                                          | A container can outlive the chat or job that owned it by the length of one teardown. The idle reaper and `pnpm ws:reap` collect it; nothing else is left behind.                           |
