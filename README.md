...
| **A workspace left `STOPPING` by a dead process is recovered only at worker boot** (R39). Nothing in the steady state can do it: a teardown, the idle selection and the reconciliation each refuse such a row, for reasons that are individually right. Closing it in the steady state needs a lease the owner renews — a column, a renewal loop and an expiry — and age cannot stand in for ownership, which this project has already been bitten by in the port-base allocator. | The row is closed out on the next worker start, which is when a crashed worker comes back, and its container is removed by the orphan pass. Between the crash and that restart the row counts as live. |
