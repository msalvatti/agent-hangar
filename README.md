...
An agent turn runs end to end. The workspace clones the repository, checks out a dedicated `agent/<id>` branch, runs its tools, and commits and pushes to that branch rather than to your default one. Everything it did is persisted as it happens, so a transcript is read back from the database on a reload with its tool calls, their arguments, their output and their exit codes intact. [Known gaps](#-known-gaps) is where the boundaries are written down, including the two things a reload does not restore.
...
- `pnpm setup` takes a few minutes on a cold machine, nearly all of it the dependency install (about 1 100 packages) and, when `agent-hangar/workspace:dev` is not on the host yet, the image build. It ends with the doctor's table and `Setup complete for instance "default"`. A second run takes seconds.
- `pnpm dev` prints one banner line, is serving in about a second, and stays in the foreground until Ctrl-C. The URL to open is the `127.0.0.1` one Next.js prints, not the `localhost` one the banner above it prints — both servers bind `127.0.0.1` only.
...
**Ports 3000, 3001 and 3002 have to be free, and `pnpm setup` does not check before it commits.** It gets as far as step 5 and then fails on the compose port bind — `Bind for 127.0.0.1:3001 failed: port is already allocated` — leaving a half-created compose project behind. If that block is taken, choose another one on the first run instead:
...
`lsof -nP -iTCP:3000 -sTCP:LISTEN` names whoever holds a port, and `pnpm infra:down` clears the containers a failed run left. Several checkouts run side by side this way — see [Working with Conductor](#-working-with-conductor).

### Three names pnpm answers for itself

`pnpm <name>` normally runs the package script of that name, but pnpm ships commands of its own, and where the two collide the script does not always win. Measured on the pinned pnpm 11.22.0:
...
Eight steps, seven of them numbered in its output. It is idempotent — a second run reports `already exists`, `master key present` and `workspace image present`, changes nothing and exits 0. Nothing below overwrites an existing file unless you pass a flag.
...
**A real-stack run passes seven of its nine checks.** `cancel-turn` never finds a Stop button, because the `sleep 60` it relies on to hold the turn open is recorded as failed within milliseconds and the turn is over in under half a second; `chat-archive-restore` waits for wording the mock API uses and the product does not. Both are recorded rather than adjusted away — see [Known gaps](#-known-gaps).
...
| `pnpm setup` stops at step 5 on `port is already allocated`       | Something else holds 3001 or 3002. `pnpm infra:down` clears the half-created project, then start the instance on a free block: `AH_INSTANCE=mine AH_PORT_BASE=3900 pnpm run setup --force`.                                             |
...
| `[WARN] Unsupported engine` during the install                    | You are not on Node 24, and the install continues anyway. `nvm use` or `fnm use` reads `.nvmrc`; anything later will fail in a less obvious place.                                                                                      |
...
| **The workspace image tag is machine-global** (R29). `WORKSPACE_IMAGE` derives to `agent-hangar/workspace:dev` for every instance, so `pnpm infra:image` in one checkout retargets the tag every other checkout resolves at container creation. Containers that already exist are bound to the image they were created from and are unaffected.                                            | Two checkouts of different revisions can run each other's runtime. Give one of them its own tag: `WORKSPACE_IMAGE=agent-hangar/workspace:mine pnpm infra:image`.                           |
| **The image-presence check is optimistic before the first run** (R10). The runner port now exposes `imageExists`, but the boot check and the health card still report what the last `create` observed.                                                                                                                                                                                     | The health card can show a green image before anything has ever been created. `pnpm infra:doctor` asks Docker directly.                                                                    |
| **Two things a reloaded transcript does not restore.** A failed non-streaming tool call renders its output on stdout after a reload although it was written to stderr live, so the border marking it is lost (R44); and the `Prepared agent/<branch> at <sha>` notice is live-only by design (R48).                                                                                        | Tool calls, their arguments, their output, their exit codes, the push notice and the cancellation notice all survive. These two do not.                                                    |
| **A `JobRun` has no message channel** (R46), so a push or preparation notice inside a scheduled run cannot be persisted the way a chat's is; and a chat renders only its newest turn's error, so an earlier failed turn loses its failure row on reload.                                                                                                                                   | A scheduled run's drawer shows its transcript and its raw output, not those notices; an older failure is visible in the live session only.                                                 |
...
| Item                                                                                                                                                                                                                                                                                                                        | Residual risk                                                                                                                                                     |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Two end-to-end checks fail against a real stack.** `cancel-turn` finds no Stop button, because the `sleep 60` meant to hold the turn open is recorded failed within milliseconds and the turn ends in under half a second (R34); `chat-archive-restore` waits for the mock API's wording rather than the product's (R35). | Seven of nine pass. Cancellation and restore are exercised in mock mode and by unit suites, but no automated check covers them end to end against the real stack. |
...
| **No check in this repository can see one interactive control covering another** (R43). Every `getBoundingClientRect` in jsdom is 0×0, so an overlap assertion written there passes whatever the layout does, and three real collisions were found by eye rather than by a suite.                                           | Layout regressions of that class reach the running product.                                                                                                       |
| **A class-name assertion can pass while the class loses** (R42). A component asking for a width the primitive overrides still satisfies a test that only asserts the class is present.                                                                                                                                      | The same shape has appeared five times; it is a pattern to distrust, not a single defect.                                                                         |
...
| **`pnpm setup --force` silently drops the flag**, and two of the scripts print exactly that spelling as their own remedy — for an incomplete `.env.local`, and for a shell that names a different instance than the file does.                                                                   | Following the printed advice literally fails the same way twice. `pnpm run setup --force` is the form that works.                                       |
| **The Node requirement is a warning, not a gate.** `engines` asks for `>=24 <25` and `.npmrc` sets `engine-strict=true`, but pnpm 11 does not read that setting — `pnpm config get engine-strict` answers `undefined` — so a wrong major prints `[WARN] Unsupported engine` and installs anyway. | The failure surfaces later, somewhere less obvious. Check `node -v` yourself, or run `nvm use` / `fnm use`.                                             |
| **`pnpm dev` and the doctor print `localhost` URLs** while both servers bind `127.0.0.1` only. Most clients fall back to IPv4 and connect; one that resolves `localhost` to `::1` and stops there does not.                                                                                      | Use the `127.0.0.1` URL, which is what every generated connection string uses.                                                                          |
