...
>
> **Not finished:** a final wiring-and-stabilisation pass, and mutation testing — **deferred by decision on 2026-08-20**, still in the plan and scheduled for later rather than dropped. The Playwright end-to-end suite is now on `main`: the mocked run is green in continuous integration, and a real-stack run passes 7 of its 9 checks. See [Known gaps](#-known-gaps) — that section is specific rather than reassuring, including about the things it would be more comfortable to leave out.
>
> **What has been exercised by hand:** `pnpm setup` from a clean tree, `pnpm dev` bringing up web and worker together, every page rendering, `GET /api/health` reporting Postgres, Redis, Docker and the workspace image healthy, and the `@db` / `@redis` / `@docker` integration suites green against real services.
>
> **A full agent turn against a real repository with a real model key has now been recorded**, on 2026-08-20: a chat on a GitHub repository cloned, prepared a dedicated `agent/<id>` branch, ran `list_dir` and `read_file` (both exit 0) and answered from the real model — `SUCCEEDED`, 2 steps, 1435 input and 147 output tokens. A scan of every text column of every table other than the secrets table found no credential-shaped string. Until that day the path had never completed at all: the model provider was not wired into the binary the container runs, so every real turn failed at composition.
...
| Item                              | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **End-to-end suite** (Playwright) | On `main` and running. The mocked project is green in continuous integration. A real-stack run passes **7 of 9**: `cancel-turn` finds no Stop button because a `sleep 60` shell call was recorded `FAILED` in 2 ms and the turn finished in 408 ms, and `chat-archive-restore` waits for wording the double uses and the product does not. Both are recorded as findings — R34 and R35 in the plan's routed table — rather than adjusted away.                             |
...
| **Mutation testing** (Stryker 10) | **Deferred by decision on 2026-08-20 — still in the plan, scheduled later, not dropped.** Scope and thresholds are already fixed (`break: 80`, target 90, over secrets, redaction, scheduling, workspace lifecycle, restore, the protocol codec and the runtime tools). No package defines `test:mutation` yet, so the root script is a no-op and there is no CI job. It was never a release gate: the plan says in as many words that the product is complete without it. |
